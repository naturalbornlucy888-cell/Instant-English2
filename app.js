...
const NEWS_COUNT  = 7;
const REVIEW_MAX  = 3;
const REVIEW_MODE_COUNT = 5;   // 復習モードの出題数
const GRADUATE_COUNT    = 2;   // 復習モードで何回正解したら卒業か

const STORAGE_KEYS = {
  NEWS_KEY:      'iet_news_key',
  REVIEW_POOL:   'iet_review_pool',   // { ja, en, correctCount, nextDate }[]
};
...
const SAMPLE_SENTENCES = [
  ...
  { ja: '地震の被災地では復習作業が続いている。',                           en: 'Recovery work continues in areas affected by the earthquake.' },
...
// ---------- 復習プール（localStorage） ----------
// 構造: { ja, en, correctCount, addedDate }[]
function getReviewPool() {
  try { return JSON.parse(localStorage.getItem(STORAGE_KEYS.REVIEW_POOL)) || []; }
  catch { return []; }
}
function saveReviewPool(pool) {
  localStorage.setItem(STORAGE_KEYS.REVIEW_POOL, JSON.stringify(pool));
}

// 問題を復習プールに追加（重複なし）
function addToReviewPool(ja, en) {
  const pool = getReviewPool();
  if (pool.some(r => r.ja === ja)) return; // 既存ならスキップ
  pool.push({ ja, en, correctCount: 0, addedDate: todayStr() });
  saveReviewPool(pool);
}

// 正解を記録し、GRADUATE_COUNT回正解したら卒業
function recordCorrectInPool(ja) {
  const pool = getReviewPool();
  const item = pool.find(r => r.ja === ja);
  if (!item) return;
  item.correctCount = (item.correctCount || 0) + 1;
  // 卒業判定
  const graduated = item.correctCount >= GRADUATE_COUNT;
  const newPool = graduated ? pool.filter(r => r.ja !== ja) : pool;
  saveReviewPool(newPool);
  return graduated;
}
...
// ---------- NewsAPI取得（毎回新しい記事） ----------
async function fetchNewsQuestions(newsKey, count) {
  // pageSize を多めに取り、ランダムにシャッフルして毎回違う記事を選ぶ
  const url = `https://newsapi.org/v2/top-headlines?language=en&pageSize=40&apiKey=${newsKey}`;
  const res = await fetch(url);
  if (!res.ok) {
  ...
  const articles = shuffle(
    (data.articles || []).filter(a =>
      a.title && a.title.length >= 20 && a.title.length <= 120
      && !a.title.includes('[Removed]')
    )
  ).slice(0, count * 2); // 翻訳失敗に備えて多めに

  if (articles.length === 0) throw new Error('有効な記事が取得できませんでした');

  const results = await Promise.allSettled(
    articles.slice(0, count).map(async (a) => {
      const ja = await translateToJa(a.title);
      return { ja, en: a.title, source: `📰 ${a.source?.name || 'News'}`, isReview: false };
    })
  );

  const questions = results.filter(r => r.status === 'fulfilled').map(r => r.value);
...
function getFallbackQuestions(count) {
  return shuffle(SAMPLE_SENTENCES).slice(0, count)
    .map(s => ({ ...s, source: '📚 サンプル', isReview: false }));
}

// ---------- 学習モード問題準備 ----------
async function prepareStudyQuestions(newsKey, onProgress) {
  // 復習プールから最大3問（correctCount が少ない順に優先）
  const pool = getReviewPool()
    .sort((a, b) => (a.correctCount || 0) - (b.correctCount || 0));
  const reviewQs = pool.slice(0, REVIEW_MAX).map(r => ({
    ja: r.ja, en: r.en,
    source: `🔁 復習 (${r.correctCount || 0}/${GRADUATE_COUNT}回正解)`,
    isReview: true,
  }));

  // NewsAPIから毎回新しい7問
  onProgress('最新ニュースを取得中...');
  let newsQs;
  try {
    newsQs = await fetchNewsQuestions(newsKey, NEWS_COUNT);
  } catch (e) {
    console.warn('NewsAPI失敗:', e.message);
    onProgress('サンプル問題を準備中...');
    newsQs = getFallbackQuestions(NEWS_COUNT);
  }
...
// ---------- 復習モード問題準備 ----------
function prepareReviewQuestions() {
  const pool = getReviewPool();
  if (pool.length === 0) return [];
  // correctCount が少ない順 → シャッフルで最大5問
  const sorted = pool.sort((a, b) => (a.correctCount || 0) - (b.correctCount || 0));
  return sorted.slice(0, REVIEW_MODE_COUNT).map(r => ({
    ja: r.ja, en: r.en,
    source: `🔁 復習 (${r.correctCount || 0}/${GRADUATE_COUNT}回正解)`,
    isReview: true,
  }));
...
const WORD_DICT = {
  ...
  amid:         { ja: '〜の中で', ex: 'She stayed calm amid the chaos.' },
  ...
  conclude:     { ja: '締結する', ex: 'They concluded a trade agreement.' },
  ...
  decline:      { ja: '低下する', ex: 'Sales declined last quarter.' },
  ...
  expand:       { ja: '拡大する', ex: 'The company plans to expand its services.' },
  extreme:      { ja: '極端な、異常な', ex: 'Extreme weather events are increasing.' },
  fall:         { ja: '下落する', ex: 'Prices tend to fall in winter.' },
  fell:         { ja: '下落した', ex: 'The temperature fell overnight.' },
  ...
  grow:         { ja: '増加する', ex: 'The economy continues to grow.' },
  growing:      { ja: '高まっている', ex: 'There is growing concern about pollution.' },
  ...
  intractable:  { ja: '難治性の', ex: 'Intractable diseases are hard to treat.' },
  invest:       { ja: '投資する', ex: 'We should invest in education.' },
  investment:   { ja: '投資', ex: 'Investment in education is crucial.' },
  increase:     { ja: '増加', ex: 'There has been an increase in demand.' },
  launch:       { ja: '開始する', ex: 'They will launch a new product.' },
  launched:     { ja: '開始した', ex: 'The app was launched last month.' },
  legacy:       { ja: '遺産', ex: 'The legacy of the war still remains.' },
  mandatory:    { ja: '義務的な', ex: 'Wearing a helmet is mandatory.' },
  ...
  probe:        { ja: '探査機', ex: 'A space probe was sent to Mars.' },
  ...
  significant:  { ja: '大幅な', ex: 'There was a significant improvement.' },
  significantly:{ ja: '大幅に', ex: 'Costs have significantly decreased.' },
  ...
  supply:       { ja: '供給', ex: 'Supply chain issues caused delays.' },
  transmit:     { ja: '送信する', ex: 'The signal was transmitted.' },
  transmitted:  { ja: '送信した', ex: 'Data was transmitted from Mars.' },
  treaty:       { ja: '条約', ex: 'A peace treaty was signed.' },
  urgent:       { ja: '急務の', ex: 'This is an urgent matter.' },
...
function lookupWord(word) {
  ...
  const stems = [
    clean.replace(/ations$/, ''), clean.replace(/ation$/, ''),
    clean.replace(/ments$/, ''),  clean.replace(/ment$/, ''),
    clean.replace(/ings$/, ''),   clean.replace(/ing$/, 'e'),
    clean.replace(/ing$/, ''),    clean.replace(/tions$/, 't'),
    clean.replace(/tion$/, 't'),  clean.replace(/ed$/, 'e'),
    clean.replace(/ed$/, ''),     clean.replace(/ly$/, ''),
    clean.replace(/s$/, ''),      clean.replace(/es$/, 'e'),
  ];
...
function gradeAnswer(userRaw, modelRaw) {
  ...
  const userWords  = normalize(userRaw).split(' ').filter(Boolean);
  const modelWords = normalize(modelRaw).split(' ').filter(Boolean);
  const modelWordsRaw = modelRaw.replace(/'\w+/g, '').replace(/[^\w\s]/g, ' ')
  ...
  const stopWords = new Set([
    ...
    'i','we','they','he','she','you','our','their','his','her','my','your',
  ]);
  ...
  modelWords.forEach((normWord, idx) => {
    if (!userSet.has(normWord) && !stopWords.has(normWord) && !seenNorm.has(normWord)) {
      ...
      missing.push((modelWordsRaw[idx] || normWord).toLowerCase());
    }
  });
  ...
  const comments = [];
  if (score === 5)      comments.push('完璧です！模範解答とほぼ一致しています。');
  else if (score === 4) comments.push('ほぼ正解です。細かい表現の違いがあります。');
  else if (score === 3) comments.push('概ね伝わりますが、いくつか改善点があります。');
  else                  comments.push('模範解答と大きく異なります。下の模範解答を確認しましょう。');

  if (missing.length > 0)
    comments.push(`使えていないキーワード: ${missing.slice(0,4).map(w=>`"${w}"`).join('、')}`);
  if (extra.length > 0 && score < 4)
    comments.push(`模範解答にない単語: ${extra.slice(0,3).map(w=>`"${w}"`).join('、')}`);
...
function initSpeechRecognition() {
  ...
  rec.onresult = (e) => {
    ...
    if (final) { stopRecording(); showTranscript(final.trim()); }
  };
  rec.onerror = (e) => {
    stopRecording();
    const msg = document.getElementById('mic-status-msg');
    if (e.error === 'not-allowed')
      msg.textContent = '⚠️ マイクへのアクセスが拒否されました。ブラウザの設定から許可してください。';
...
function startRecording() {
  if (!state.recognition) { switchTab('text'); return; }
...
function submitAnswer(userAnswer) {
  if (!userAnswer.trim()) { alert('回答を入力してください。'); return; }
  ...
  // 復習プール更新
  if (isCorrect) {
    // 正解 → 復習プールの正解カウントを増やす（卒業判定）
    recordCorrectInPool(q.ja);
  } else {
    // 不正解 → 復習プールに追加（学習モードの新問題も追加）
    addToReviewPool(q.ja, q.en);
  }
  ...
  const wordCards = grade.missingWords.map(w => {
  ...
  const wordSectionHtml = wordCards
    ? `<div class="result-section"><label class="field-label">📖 使えていなかった単語</label><div class="word-cards">${wordCards}</div></div>`
    : '';
  ...
  document.getElementById('result-area').innerHTML = `
    ...
    <div class="result-section">
      ...
      <div class="correction-text">${correctedHtml}<p>${escHtml(grade.comment)}</p></div>
    </div>
    <div class="result-section">
      <label class="field-label">💡 模範解答</label>
      <div class="model-answer-row">
        <p class="model-answer">${escHtml(q.en)}</p>
        <button class="btn-speak" id="speak-btn">🔊 読み上げ</button>
      </div>
    </div>
    ${wordSectionHtml}
  ...
  document.getElementById('speak-btn').addEventListener('click', () => speak(q.en));
...
function showComplete() {
  ...
  const poolSize = getReviewPool().length;
  ...
  if (state.mode === 'study') {
    note = wrong > 0
      ? `${wrong}問を復習プールに追加しました。復習プール合計: ${poolSize}問`
      : '全問正解！素晴らしいです。';
  } else {
    note = `復習プール残り: ${poolSize}問。${GRADUATE_COUNT}回正解した問題は卒業します。`;
  }
...
function init() {
  ...
  // 復習プールの件数を表示
  updateReviewPoolBadge();

  state.recognition = initSpeechRecognition();
  checkMicSupport();

  // モード選択ボタン
  document.getElementById('btn-study-mode').addEventListener('click', async () => {
    ...
    try {
      state.questions    = await prepareStudyQuestions(newsKey, msg => {
  ...
  document.getElementById('btn-review-mode').addEventListener('click', () => {
    const pool = getReviewPool();
    if (pool.length === 0) {
      alert('復習プールに問題がありません。\nまず学習モードで問題を解いてください。');
      return;
    }
    state.mode         = 'review';
    state.questions    = prepareReviewQuestions();
    state.currentIndex = 0;
    state.results      = [];
    showScreen('practice-screen');
    renderQuestion();
...
function updateReviewPoolBadge() {
  const count = getReviewPool().length;
  const el = document.getElementById('review-pool-count');
  if (el) el.textContent = count > 0 ? `復習プール: ${count}問` : '復習プールは空です';
}
